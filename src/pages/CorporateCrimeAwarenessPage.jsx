    ...
    <div style={{ background: BG, minHeight: "100vh", color: "#33404F", fontFamily: "'Inter',sans-serif" }}>
      ...
      <section style={{ padding: "90px 32px 70px", textAlign: "center", background: `linear-gradient(160deg, #FFFFFF 0%, #EAF5FD 60%, #DBEEFC 100%)` }}>
        ...
        <h1 style={{ fontWeight: 900, fontSize: "2rem", lineHeight: 1.05, textTransform: "uppercase", margin: "0 0 20px" }}>
          {tr("Corporate Crime")} <span style={{ color: G }}>{tr("Awareness Program")}</span>
        </h1>
        <p style={{ maxWidth: 680, margin: "0 auto", color: BODY, fontSize: "1.05rem", lineHeight: 1.75 }}>
          {tr("In the digital era, one careless click or misplaced trust can cost a company millions. Our program is designed to protect organizations from fintech frauds, insider scams, cyber threats, and workplace misconduct through real-world education and proactive defence training.")}
        </p>
        <p style={{ maxWidth: 680, margin: "16px auto 0", color: BODY, fontSize: ".95rem", lineHeight: 1.7 }}>
          {tr("Delivered by certified forensic and cyber-risk experts, these sessions transform employees into your first line of defence — alert, informed, and legally aware.")}
        </p>
      ...
      <section style={{ padding: "80px 32px", maxWidth: 1200, margin: "0 auto" }}>
        <h2 style={{ fontWeight: 900, fontSize: "clamp(1.6rem, 3.1vw, 2rem)", textTransform: "uppercase", marginBottom: 40, textAlign: "center" }}>
          {tr("What You'll")} <span style={{ color: G }}>{tr("Learn")}</span>
        </h2>
      ...
      <section style={{ padding: "80px 32px", background: PANEL }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <h2 style={{ fontWeight: 900, fontSize: "clamp(1.6rem, 3.1vw, 2rem)", textTransform: "uppercase", marginBottom: 32, textAlign: "center" }}>
            {tr("Key")} <span style={{ color: G }}>{tr("Highlights")}</span>
          </h2>
      ...
      <section style={{ padding: "80px 32px", maxWidth: 1200, margin: "0 auto" }}>
        <h2 style={{ fontWeight: 900, fontSize: "clamp(1.6rem, 3.1vw, 2rem)", textTransform: "uppercase", marginBottom: 40, textAlign: "center" }}>
          {tr("How We")} <span style={{ color: G }}>{tr("Deliver")}</span>
        </h2>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40, alignItems: "center", marginBottom: 48 }} className="cca-deliver-grid">
          ...
          <p style={{ color: BODY, fontSize: "1rem", lineHeight: 1.8 }}>
            {tr("Every session is built around your organization's real risk profile — not a generic slideshow. We sit down with your leadership team first, understand where the exposure actually is, and design a session that leaves employees with practical habits, not just awareness.")}
          </p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 20 }}>
          {DELIVERY.map((item) => {
            const Icon = item.icon;
            return (
              <div key={tr(item.title)} style={{ background: PANEL, border: `1px solid ${BORD}`, borderRadius: 12, padding: "28px 24px", textAlign: "center" }}>
      ...
      <section style={{ padding: "90px 32px", textAlign: "center", background: `linear-gradient(160deg, #EAF5FD 0%, #FFFFFF 100%)` }}>
        <h2 style={{ fontWeight: 900, fontSize: "clamp(1.8rem, 4vw, 2rem)", textTransform: "uppercase", marginBottom: 18 }}>
          {tr("Bring This Program")} <span style={{ color: G }}>{tr("To Your Team")}</span>
        </h2>
        <p style={{ color: BODY, maxWidth: 520, margin: "0 auto 32px", fontSize: ".95rem", lineHeight: 1.7 }}>
          {tr("Reach out to schedule a session for your organization — tailored, practical, and delivered by certified experts.")}
        </p>
